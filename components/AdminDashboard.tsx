...
import { 
  LayoutDashboard, Users, Scissors, LogOut, Trash2, Plus, Calendar, 
  DollarSign, Clock, Star, Search, Loader2, AlertCircle, CalendarOff, Info
} from 'lucide-react';
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (activeTab === 'bookings') {
          const data = await storageService.getAllBookings();
          setBookings(data);
  ...
  const handleAddSubmit = async (e: React.FormEvent) => {
    ...
    if (activeTab === 'services') {
      ...
      success = await storageService.addBarber({
        ...
        success = await storageService.addBarberAvailability(
    ...
    <div className="min-h-screen bg-dark-900 text-white flex flex-col">
      ...
      <header className="bg-dark-800 border-b border-dark-700 px-6 py-4 flex items-center justify-between sticky top-0 z-50">
        <div className="flex items-center gap-2">
          ...
          <h1 className="font-bold text-lg">GantengMaksimal <span className="text-gray-500 font-normal">| Admin</span></h1>
        </div>
            ...
            {activeTab !== 'bookings' && (
              ...
              >
                <Plus className="w-4 h-4 mr-2" /> Tambah {
                    ...
                    activeTab === 'barbers' ? 'Barber' : 'Libur'
                }
              </Button>
            )}
          </div>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                      ...
                      <tbody className="divide-y divide-dark-700">
                        {bookings.length === 0 && (
                          <tr><td colSpan={5} className="p-8 text-center text-gray-500">Belum ada booking masuk.</td></tr>
                        )}
                        {bookings.map((b) => (
                          <tr key={b.id} className="hover:bg-dark-700/50 transition-colors">
                            ...
                            <td className="p-4 max-w-xs truncate text-gray-400" title={b.style_notes}>
                              {b.style_notes || '-'}
                            </td>
                          </tr>
                        ))}
                  ...
                  {services.map(s => (
                    <div key={s.id} className="bg-dark-800 p-4 rounded-xl border border-dark-700 flex justify-between items-start">
                      <div>
                        ...
                        <div className="mt-2 text-xs bg-dark-700 inline-block px-2 py-1 rounded">
                    ...
                    <div key={b.id} className="bg-dark-800 rounded-xl border border-dark-700 overflow-hidden">
                      ...
                      <div className="p-4">
                        ...
                        <div className="flex justify-between text-xs text-gray-400 border-t border-dark-600 pt-2">
                          ...
                          <span className="flex items-center gap-1 text-yellow-400"><Star className="w-3 h-3 fill-current"/> {b.rating}</span>
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              {activeTab === 'schedule' && (
                <div className="space-y-6">
                            ...
                            {barbers.map(b => (
                                <button
                                    ...
                                    className={`flex-shrink-0 px-4 py-2 rounded-lg border transition-all flex items-center gap-2 ${
                                        selectedBarberForSchedule === b.id 
                    ...
                    {selectedBarberForSchedule ? (
                        ...
                        <div className="p-10 text-center text-gray-500 bg-dark-800 rounded-xl border border-dark-700 border-dashed">
                            Silahkan pilih barber di atas untuk melihat jadwalnya.
                        </div>
                    )}
                </div>
              )}
        ...
        <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-dark-800 border border-dark-600 p-6 rounded-2xl w-full max-w-md">
            <h3 className="text-xl font-bold mb-4 text-white">
                {activeTab === 'schedule' ? 'Tambah Hari Libur' : 
                 activeTab === 'services' ? 'Tambah Layanan' : 'Tambah Barber'}
            </h3>
            <form onSubmit={handleAddSubmit} className="space-y-4">
              ...
              {activeTab === 'services' && (
                <>
                   <div>
                    ...
                    <input required className="w-full bg-dark-900 border border-dark-600 p-2 rounded text-white" 
                      onChange={e => setFormData({...formData, name: e.target.value})} />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="text-sm text-gray-400">Harga (Rp)</label>
                      <input type="number" required className="w-full bg-dark-900 border border-dark-600 p-2 rounded text-white" 
                  ...
                  <div>
                    ...
                    <textarea required className="w-full bg-dark-900 border border-dark-600 p-2 rounded text-white" 
              ...
              {activeTab === 'barbers' && (
                <>
                  ...
                  <div>
                    ...
                    <input className="w-full bg-dark-900 border border-dark-600 p-2 rounded text-white" placeholder="https://..."
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input type="number" step="0.1" max="5" required className="w-full bg-dark-900 border border-dark-600 p-2 rounded text-white" 
              ...
              {activeTab === 'schedule' && (
                <>
                    ...
                    <div>
                        ...
                        <input type="date" required className="w-full bg-dark-900 border border-dark-600 p-2 rounded text-white" 
                            onChange={e => setFormData({...formData, date: e.target.value})} />
                    </div>
                    <div>
                        <label className="text-sm text-gray-400">Alasan</label>
                        <input required className="w-full bg-dark-900 border border-dark-600 p-2 rounded text-white" placeholder="Cuti / Sakit / Libur Rutin"
              ...
              <div className="flex gap-3 mt-6 pt-4 border-t border-dark-700">
                <Button type="button" variant="secondary" className="flex-1" onClick={() => setIsAddModalOpen(false)}>Batal</Button>
